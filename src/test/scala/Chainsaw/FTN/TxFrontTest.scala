...
class TxFrontTest extends AnyFlatSpec {
  ...
  val qamPositions = loadFTN[Array[Double]]("QAMPositions").map(_.toInt).map(_ - 1)
  logger.info(s"qam position: ${qamPositions.mkString(" ")}")
  val qamRemapPositions = loadFTN[Array[Double]]("QAMRemapPositions").map(_.toInt).map(_ - 1)

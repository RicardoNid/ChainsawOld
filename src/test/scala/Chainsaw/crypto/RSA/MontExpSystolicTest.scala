...
case class MontExpTestCase(modeId: Int, changeKey: Boolean = true, useGivenValue: Boolean = false, X: BigInt = 0, M: BigInt = 0, E: BigInt = 0)
...
class MontExpSystolicTest extends AnyFunSuite {
  ...
  test("testMontExpSystolic") {
    ...
    val badCase = MontExpTestCase(
      2,
      true,
      true,
      BigInt(
        ""
      ),
      BigInt(
        ""
      ),
      BigInt("21")
    )
      ...
      SimConfig.withWave.workspaceName(name).compile(new MontExpSystolic(testConfig)).doSim { dut =>
        ...
        def runTestCases(montExpTestCases: Seq[MontExpTestCase]) = {
          montExpTestCases.foreach { testcase =>
            ...
            ref.refresh()
            val testModulus =
              if (testcase.useGivenValue) testcase.M
              else if (testcase.changeKey) BigInt(ref.getModulus)
              else lastModulus
            ...
            val testInputs = (0 until currentInstanceNumber).map(_ =>
              if (testcase.useGivenValue) testcase.X
              else BigInt(ref.getPrivateValue) / ChainsawRand.nextInt(10000) - ChainsawRand.nextInt(10000)
            )
            // get words
            require(testExponent % 2 == 1)
            val testExponentWords =
              testExponent.toString(2).padTo(currentTestSize, '0').grouped(testWordSize).toArray.map(wordString => BigInt(wordString.reverse, 2))
            ...
            def runForOnce() = {
              ...
              val starterIds = (0 until parallelFactor)
                .filter(_ % groupPerInstance(modeId) == 0)
              ...
              def montExpResultMonitor() = if (io.valids(0).toBoolean) dutResults.zip(currentDataOuts).foreach { case (buffer, signal) =>
                buffer += signal.toBigInt
              }
              ...
              if (comparStageByStage)
                MontAlgos.Arch1ME(testInputs(0), testExponent, testModulus, testWordSize, print = true) // print the partial products

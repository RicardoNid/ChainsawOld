...
class HuaweiKyberTest extends AnyFlatSpec {
  ...
  it should "make sure that fast NTT is still NTT" in assert(
    NTT(a).sorted.diff(FNTT(a).sorted).isEmpty
  ) // only require them to be the same as sets, as the order is different
  ...
  it should "make sure that NWCByNTT is still negative wrapped convolution correctly" in assert(
    NWCByNTT(a, b).diff(polyRing.negativeWrappedConvolution(a, b)).isEmpty
  )
  ...
  val testPairs: Seq[Seq[BigInt]] =
    (0 until 10000).map(_ => BigInt(ChainsawRand.nextInt(p))).grouped(2).toSeq.map(vec => Seq(vec(0), vec(1), BigInt(randomOmega)))
  ...
  "the KNTT hardware" should "pass the random test" in {
    ...
    testDSPNode[UInt, Seq[BigInt], BigInt](nttDFG.asNode("ntt", log2Up(N) * 5 cycles), Seq.fill(N)(12 bits), knttTestCase, knttGolden.flatten)
  }
  ...
  "the IKNTT hardware" should "pass the random test" in {
    testDSPNode[UInt, Seq[BigInt], BigInt](inttDFG.asNode("intt", log2Up(N) * 5 cycles), Seq.fill(N)(12 bits), knttTestCase, kinttGolden.flatten)
  }
}

...
class MontExpSystolicTest extends AnyFunSuite {
  ...
  test("testMontExpSystolic") {
    ...
    def standardTestCases(config: MontConfig) = config.lMs.indices.map(i => Seq(MontExpTestCase(i), MontExpTestCase(i, false))).flatten
    ...
    if (doSim) {
      (0 until simTimes).foreach(_ => sim("temp", testConfig, standardTestCases(testConfig)))
    }
    if (doSynth) VivadoSynth(new MontMulPE(testConfig.w))

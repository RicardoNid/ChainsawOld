...
/** @param inputRealInfo
  *   all the numeric information of the signal(interval and error), this is a var, so it can be reassigned, determines MSB
  * @param resolution
  *   the number of bits used for fractional part, determines LSB
  * @param withRoundingError
  *   if enabled, the signal will be initialized with rounding error, which is determined by the resolution
  * @note
...
class Real(var realInfo: RealInfo, val qWidths: QWidths) extends MultiData {
  ...
  /** Operations
    *
    * @note
    *   operations on Real are defined in the following aspects
    *
  ...
  def doAddSub(that: Real, add: Boolean): Real = {
    ...
    val retRaw =
      if (this.isPositive && that.isPositive) {
        ...
        else if (add) rawLeft.asUInt + rawRight.asUInt
        else rawLeft.asUInt - rawRight.asUInt
      }.asSInt
      else {
        if (maxExpEnlarged) if (add) rawLeft +^ rawRight else rawLeft -^ rawRight
        else if (add) rawLeft + rawRight
        else rawLeft - rawRight
      }
  ...
  /** Operation with constant
    *
    * @param thatConstant
    *   the constant which share the same resolution as the signal,
    *
      ...
      case that if this.getClass.isAssignableFrom(that.getClass) =>
        ...
        if (this.maxExp < t.maxExp) { // overflow is not allowed
          if (!t.hasTag(tagTruncated)) {
            ...
            globalData.pendingErrors += (() =>
              s"$this can't be assigned by \n" +

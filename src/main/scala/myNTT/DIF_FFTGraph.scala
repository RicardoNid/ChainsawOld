...
class DIF_FFTGraph[T <: Data](name: String,
                              dataType: HardType[T],
  ...
  require(isPow2(N) && N > 1 && constArray.length == (N >> 1) &&
  ...
  //内部连线
  for (i <- 0 until n) {
    val switchOn1 = (0 until p(i)._1).map(j =>
      Schedule((j + delay) % (p(i)._1 << 1), p(i)._1 << 1))
    val switchOff1 = (0 until p(i)._1).map(j =>
      Schedule((j + p(i)._1 + delay) % (p(i)._1 << 1), p(i)._1 << 1))
    val constantSw = p(i)._2(0).indices.map(j =>
      Schedule((delay + p(i)._1 + j) % p(i)._2(0).length, p(i)._2(0).length))
    val switchOn2 = (0 until p(i)._3).map(j =>
      Schedule((j + delay + p(i)._1 + butterflyHardware.delay) %
        (p(i)._3 << 1), p(i)._3 << 1))
    val switchOff2 = (0 until p(i)._3).map(j =>
      Schedule((j + p(i)._3 + delay + p(i)._1 + butterflyHardware.delay) %
        (p(i)._3 << 1), p(i)._3 << 1))
    for (j <- 0 until (IONum >> 1)) {
      addEdge(dataPresent(linkList(2 * j)), switchNodes(2 * i)(j)(0),
        if (withBeforeNum(linkList(2 * j))) beforeNum else 0)
      addEdge(dataPresent(linkList(2 * j + 1)), switchNodes(2 * i)(j)(1),
        if (withBeforeNum(linkList(2 * j + 1))) beforeNum + p(i)._1 else p(i)._1)
  ...
  for (i <- 0 until IONum) {
    ...
    addEdge(dataPresent(linkList(i)), dataOut(i)(0),
      if (withBeforeNum(linkList(i))) beforeNum else 0)
  }

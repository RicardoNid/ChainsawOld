...
class NTT(N: BigInt, //被模常数
  ...
  if (foldNum != 1) {
    ...
    val foldSet = new Array[Seq[DSPNode[UInt]]](
      (IONum >> 1) * 3 * log2Up(numOfNTT) / foldNum)

...
case class CordicData() extends Bundle {
  val x = SFix(1 exp, -14 exp) // 1QN
  val y = SFix(1 exp, -14 exp) // 1QN
  val z = SFix(2 exp, -13 exp) // 2QN
}
...
class CORDICSim(cordicConfig: CordicConfig) extends Component with DSPSim {
  ...
  val cordic = CORDIC(input.payload.x, input.payload.y, input.payload.z, cordicConfig)
  ...
  override type testCaseType = CordicSimData
  override type testResultType = CordicSimData

  override def poke(testCase: testCaseType, input: inputType): Unit = {
  ...
  override def messageWhenInvalid(testCase: testCaseType, refResult: CordicSimData, dutResult: CordicSimData): String =
    s"\n[ERROR]\ninput: ${testCase}\ngolden: ${refResult}\nyours: ${dutResult}"

  override def messageWhenValid(testCase: testCaseType, refResult: CordicSimData, dutResult: CordicSimData): String = ""
...
object CORDICSim {
  def randomCase(cordicConfig: CordicConfig) = {
    val r = Random

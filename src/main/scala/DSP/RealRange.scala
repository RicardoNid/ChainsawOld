//package DSP
//
//import scala.math.{abs, exp, max, min, pow}
//
...
//  }
//
//  def getBitCount = getMaxExp - getMinExp
//
...
//  }
//
//  def unary_-() = new RealRange(-this.lower, -this.upper, this.resolution)
//
//  override def toString: String = s"RealRange: [$lower, $upper], resolution = $resolution, maxExp = $getMaxExp, minExp = $getMinExp, bitCount = $getBitCount"
//
...
//  def -(constant: BigDecimal) = new RealRange(this.lower - constant, this.upper - constant, this.resolution - constant)
//
//  def *(constant: BigDecimal) = new RealRange(this.lower * constant, this.upper * constant, (this.resolution * constant).abs)
//
//  def ^(exponent: Int) = {
//    require(exponent > 0)
//    //    Array.fill(exponent)(this).reduce(_ * _)
//    if (this.contains(0) && exponent % 2 == 0) new RealRange(0.0, pow(this.upper.abs max this.lower.abs, exponent), pow(this.resolution, exponent))
//    else new RealRange(pow(this.lower, exponent), pow(this.upper, exponent), pow(this.resolution, exponent))
//  }
//
//  def <<(shift: Int) = {

...
package object Chainsaw extends RealFactory {
  ...
  implicit class SpinalLiterals(private val sc: StringContext) {
    ...
    /** Invoke QFormatParser and make using QFormat easier
      *
      * @example
      *   val SF1Q3 = HardType(SFix("1Q3"))
      */
    def Q(args: Any*): QFormat = QFormatParser(getString(args))

    /** Q Format literal, we follow the same definition as Xilinx, where MQN stands for a (M + N + 1) bits signed fixed point number with N fractional bits and
      * M integer bits
      *
      * @see
  ...
  implicit class SimRealPimper(r: Real) {
    ...
    def #=(value: BigDecimal): Unit = {
      ...
      val shift = -r.minExp
      val ret =
        if (shift >= 0) // ret this is the "binary string" of value at specific precision
    ...
    /** Generate all possible value under current range, for simulation
      *
      * @example
      *   for (-0.3,0.4,0.1) whose ulp = 0.0625, numbers generated are
      */
  ...
  implicit class BinaryStringUtil(s: String) {
    ...
    def toSigned: BigInt =
      s.tail.reverse.zipWithIndex.map { case (c, i) => c.asDigit * (BigInt(1) << i) }.sum - s.head.asDigit * (BigInt(1) << s.length - 1)
  }
  ...
  implicit class RandomUtil(rand: Random) {
    ...
    def nextComplex(min: Double = 0, max: Double = 1): BComplex = new BComplex(rand.nextDouble() * (max - min) + min, rand.nextDouble() * (max - min) + min)
  ...
  /** following methods are designed for doing synth/impl through command-line for these methods, the results are exported to a default dir in this project, the
    * sub dir can be specific by name field
    */
  ...
  val synthWorkspace = "synthWorkspace"
  ...
  def toWordStrings(value: BigInt, wordLength: Int, wordCount: Int): Seq[String] = {
    value
      .toString(2)
      .padToLeft(wordLength * wordCount, '0')
      .grouped(wordLength)
      .toSeq
      .takeRight(wordCount)
      .reverse

//package Chainsaw
//
//import Chainsaw.FIR.{DAFIR, systolicFIR}
...
//
//class FIR(input: Real, coefficients: IndexedSeq[Double], firArch: FIRArch) extends ImplicitArea[Real] with DSPDesignOld {
//
...
//  val result = Real(input.maxExp + bitWidthGrowth exp, input.minExp exp)
//  var delay = 0
//
//  firArch match {
//    case FIRArch.MAC => {
...
//  }
//
//  override def implicitValue: Real = result
//
...
//  def systolicFIR(input: Real, coefficients: IndexedSeq[Double]) = {
//
//    // types
//    def typeX = QFormatReal(SQ(input.bitCount, -input.minExp))
//
//    val multBitGrowth = log2Up(ceil(coefficients.map(abs(_)).max).toInt)
//
//    def typeMult(i: Int) = Real(SQ(input.bitCount + multBitGrowth, -input.minExp))
//
//    val accBitGrowth = multBitGrowth + log2Up(coefficients.length)
//
//    def typeAcc = Real(input.maxExp + accBitGrowth exp, input.minExp exp)
//
...
//  }
//
//  def DAFIR(input: Real, coefficients: IndexedSeq[Double]) = {
//
//    // the arch below is fully-pipelined
//    val n = coefficients.length
//    val b = input.bitCount
//
//    val srl: Vec[Real] = History(input, n)
...
//  }
//
//  def apply(input: Real, coefficients: IndexedSeq[Double], firArch: FIRArch): FIR = new FIR(input, coefficients, firArch)
//}

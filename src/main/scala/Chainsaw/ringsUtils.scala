...
import cc.redberry.rings.primes.{BigPrimes, SmallPrimes}
...
object ringsUtils {
  ...
  def toPolyExpression(coeffs: Seq[Long], variable: String): String =
    coeffs.zipWithIndex.map { case (coeff, degree) => s"$coeff*$variable^$degree" }.mkString("+")

...
class MultiCountCounter(counts: Seq[BigInt], modeOH: Bits) extends ImplicitArea[UInt] {
  ...
  /** Convert this stream to a flow. It will send each value only once. It is "start inclusive, end exclusive". This means that the current value will only be
    * sent if the counter increments.
    */

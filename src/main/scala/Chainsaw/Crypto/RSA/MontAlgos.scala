...
class MontAlgos {
  ...
  def Arch1MM(X: BigInt, Y: BigInt, M: BigInt, w: Int) = {
    val n = M.bitLength
    val e = ceil((n + 1).toDouble / w).toInt
    ...
    (0 until e + n - 1).foreach { cycle =>
      ...
      //      println(s"cycle: $cycle, workings PEs: ${(peFirst to peLast).mkString(" ")}, js: ${(peFirst to peLast).map(cycle - _).mkString(" ")}")
      (peFirst to peLast).foreach { pe =>
        ...
        if (j == 0) { // task D extra work
          val s0 = SWords(0).toString(2).padToLeft(w, '0').reverse(1).asDigit
          qs(i) = (xi * y0) ^ s0
        }
        // task D & E cowork
        val tempOdd = (1 << (w - 1)) + (SWords(j) >> 1) + CWords(j) + xi * YWords(j) + qs(i) * MWords(j)
        val tempEven = (SWords(j) >> 1) + CWords(j) + xi * YWords(j) + qs(i) * MWords(j)
        val det = SWords(j + 1).toString(2).last.asDigit
        CWords(j + 1) = if (det == 1) tempOdd / (1 << w) else tempEven / (1 << w)
        SWords(j) = if (det == 1) tempOdd % (1 << w) else tempEven % (1 << w)
      }
    }
  ...
  def verifyMM(algo: (BigInt, BigInt, BigInt) => BigInt) = {
    (0 until 10).foreach { _ =>
      ...
      assert(algo(input0, input1, modulus) == BigInt(ZN.multiply(input0, input1, RhoInverse).toByteArray))
    }
...
object MontAlgos {
  def main(args: Array[String]): Unit = {
    val algo = new MontAlgos
    //    algo.verifyMM(algo.R2MM)
    //    algo.verifyMM(algo.MWR2MM(_, _, _, 4))
    algo.verifyMM(algo.Arch1MM(_, _, _, 4))

    println(algo.Arch1MM(BigInt(159), BigInt(148), 177, 4))
    val ZN = Zp(177)
    val RhoInverse = ZN.reciprocal(BigInt(1) << BigInt(177).bitLength)
    println(ZN.multiply(159, 148, RhoInverse))

    println(algo.MWR2MM(BigInt(159), BigInt(148), 177, 4))
    println(ZN.multiply(159, 148, RhoInverse))
  }
}

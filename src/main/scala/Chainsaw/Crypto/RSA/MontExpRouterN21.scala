...
case class MontExpRouterN21[T <: BitVector](config: MontConfig, hardType: HardType[T]) extends Component {
  ...
  switch(True) { // for different modes
    lMs.indices.foreach { modeId => // traverse each mode, as each mode run instances of different size lM
      ...
      is(mode(modeId))(when(work)(starterIds.foreach(j => toDes(j) := src(selectCount + j)))) // selection network version
    }
  }
}

case class MontExpRouter12N[T <: BitVector](config: MontConfig, hardType: HardType[T]) extends Component {
  ...
  switch(True) { // for different modes
    lMs.indices.foreach { modeId => // traverse each mode, as each mode run instances of different size lM
      val starterIds = (0 until parallelFactor).filter(_ % groupPerInstance(modeId) == 0) // instance indices of current mode
        .take(parallelFactor / groupPerInstance(modeId))
      is(mode(modeId))(when(work)(starterIds.foreach(j => toDes(selectCount + j) := src(j)))) // selection network version
    }
  }
}

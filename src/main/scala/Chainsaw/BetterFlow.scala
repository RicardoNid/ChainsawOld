...
object BetterFlow {
  ...
  object FlowWidthAdapter {
    def apply[T <: Data, T2 <: Data](
        input: Flow[Fragment[T]],
        output: Flow[Fragment[T2]],
      ...
      if (inputWidth == outputWidth) { // connectFrom
        ...
        endianness match {
          ...
          case `BIG` =>
            output.fragment.assignFromBits(input.fragment.asBits.resize(paddedInputWidth).subdivideIn(factor slices).reverse.read(counter))
        }
    ...
    def make[T <: Data, T2 <: Data](
        input: Stream[Fragment[T]],
        outputPayloadType: HardType[T2],
        endianness: Endianness = LITTLE,
        padding: Boolean       = false
    ): Stream[Fragment[T2]] = {

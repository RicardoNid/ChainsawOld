...
object DFGTestUtil {
  ...
  /** Verify that the transformed DFG has the same function as the original one
    *
    * @param original
    *   DFG before transformation
    * @param transformed
    *   DFG after transformation
    * @param speedUp
    *   the throughput of transformed DFG, 3 for *3, -3 for 1/3
    * @param delay
    *   extra delay on latency, latency' = latency / speedUp + delayed
    */
  def verifyFunctionalConsistency[T <: Data](
      original: DFGGraph[T],
      transformed: DFGGraph[T],
      elementType: HardType[T],
      speedUp: Int,
      testLength: Int   = 50,
      name: String      = null,
      basicLatency: Int = -1
    ...
    require(
      transformed.inputNodes.size == original.inputNodes.size * scaleFactor &&
    ...
    val originalLatency    = original.latency
    val transformedLatency = transformed.latency
    ...
    def testDFG(
        ...
        latency: Int,
        speedUp: Int,
        inputRecord: ArrayBuffer[BigInt],
        outputRecord: ArrayBuffer[BigInt],
...
//    assert(originalResults.zip(transFormedResults).forall { case (ori, trans) => ori == trans }) // FIXME: verify the unaligned part
  ...
  def verifyFolding[T <: Data](
      original: DFGGraph[T],
      foldingSets: Seq[Seq[DSPNode[T]]],
      elementType: HardType[T],
      name: String      = null,
      basicLatency: Int = -1
  ...
  def verifyUnfolding[T <: Data](original: DFGGraph[T], unfoldingFactor: Int, elementType: HardType[T], name: String = null, basicLatency: Int = -1)(implicit

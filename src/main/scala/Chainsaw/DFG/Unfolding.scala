...
class Unfolding[T <: Data](override val dfg: DFGGraph[T], unfoldingFactor: Int) extends DFGTransform[T] {
  ...
  override def constraint(sourceIteration: Iteration[T], targetIteration: Iteration[T]): DSPConstraint[T] =
    sourceIteration.device - targetIteration.device <= 0 // always valid

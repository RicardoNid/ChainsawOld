...
class RegularDFG[T <: Data](name: String) extends DFGGraph[T](name) {
  ...
  def build2D(row: Int, col: Int, node: DSPNode[T], edges: Seq[RegularEdge]): Unit = {
    ...
    /** Chain all Edges in that given direction {{{In Out Ports of a cell is symmetrical: for each input port there will be an output port}}}
      *
      * @param edgeDirection
      *   specify one edgeDirection from eight directions
      * @param InOut
      *   from the view of each cell, should each edge of such edgeDirection coming In or Out?
      * @param outOrder
      *   from the view of each cell, ...
      * @param inOrder
      *   from the view of each cell, ...
      * @param delay
      *   reg between cells
      * @param withInput
      *   whether that direction of chain has Input
      * @param withOutput
      *   whether that direction of chain has Output
      */
    def addAllEdgeChain(edgeDirection: Value, InOut: Value, outOrder: Int, inOrder: Int, delay: Int, withInput: Boolean, withOutput: Boolean): Unit = {
      ...
      println(
        s"edgeDirection = $edgeDirection, (${indexDiff._1}, ${indexDiff._2}), InOut = $InOut, withInput = $withInput, withOutput = $withOutput"
      ) // testing print
            ...
            InOut match {
              case IN =>
                if (withOutput) {
                  ...
                  this.setOutput(
                    nodes(r + indexDiff._1)(c + indexDiff._2),
                    outOrder,
                    s"output_${edgeDirection}_${r + indexDiff._1}_${c + indexDiff._2}($outOrder)",
                    Seq(Schedule(0, 1))
                  )
                  inoutNodeAttrSeq += (s"output_${edgeDirection}_${r + indexDiff._1}_${c + indexDiff._2}($outOrder)" -> List(
                    r + indexDiff._1,
                    c + indexDiff._2,
                    outOrder,
                    indexDiff._1,
              ...
              case OUT =>
                if (withInput) {
                  ...
                  this.setInput(
                    nodes(r + indexDiff._1)(c + indexDiff._2),
                    inOrder,
                    s"input_${edgeDirection}_${r + indexDiff._1}_${c + indexDiff._2}($inOrder)",
                    Seq(Schedule(0, 1))
                  )
                  inoutNodeAttrSeq += (s"input_${edgeDirection}_${r + indexDiff._1}_${c + indexDiff._2}($inOrder)" -> List(
                    r + indexDiff._1,
                    c + indexDiff._2,
                    inOrder,
                    indexDiff._1,
    ...
    edges.foreach { edgeGroup =>
      addAllEdgeChain(
        edgeGroup.edgeDirection,
        edgeGroup.InOut,
        edgeGroup.outOrder,
        edgeGroup.inOrder,
        edgeGroup.delay,
        edgeGroup.withInput,
        edgeGroup.withOutput
      )
    }
...
class RegularDFGTest extends AnyFlatSpec {
  ...
  def testSA(): Unit = {
    ...
    testDFG(
      dfg          = SAdfg,
      graphLatency = 2 cycles,
      inputWidths  = Seq.fill(4)(6 bits), // graphLatency = (row-1) + (col-1)
      testCases    = Seq(Seq.fill(4)(BigInt(1))),
      golden       = Seq(0, 4, 0, 4),
      initLength   = 0 // decide golden by yourself
    )

...
object Operators {
  ...
  /** signed adder with carry
    */
  def sintAddCNode(width: BitCount): DeviceNode[SInt] = DeviceNode(
    "sintAdderC",
    DSPHardware(
      impl = (dataIns: Seq[SInt], _: GlobalCount) => {
  ...
  /** a operator using DSP48 to finish multiply-accumulate
    *
    * @param mreg
    *   whether pipeline the multiplication result or not
    */
  @xilinxDevice
  ...
  def SACell(width: BitCount): DeviceNode[SInt] = DeviceNode(
    "SACell",
    DSPHardware(
      impl = (dataIns: Seq[SInt], _: GlobalCount) => {

...
case class RxFrontMore()
  extends RxLoop(Seq(0))
  ...
  deInterleave.dataOut
    .payloadMap(bools2bits)
    .withValid(deInterleave.dataOut.valid && Delay(fdeBuffer.iterLast, 19 + 1, init = False)) >> dataOut
}

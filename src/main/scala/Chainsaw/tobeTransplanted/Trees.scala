...
/** Binary tree(bottom-top reversed, actually), connecting vertexex by binary operations
  *
  * @param input
  *   input vector of signals with info attached to it, leaves of the tree
  * @param operator
  *   the binary operator, the hardware building block of the tree, using the signals and infos
  * @param pipelineInterval
  *   distance between registers in terms of the depth of the tree, 0 as no pipelining
  * @tparam T
  *   type of signals in input vector
  * @see
  ...
  */
class BinaryTreeWithInfo[T <: Data, I](input: IndexedSeq[(T, I)], operator: ((T, I), (T, I)) => (T, I), pipelineInterval: Int = 0)
  ...
  private def buildTree(input: IndexedSeq[(T, I)], currentDepth: Int = 0): Seq[(T, I)] = {
    ...
    width match {
      ...
      case _ =>
        ...
        // pipelining
        val pipelinedResult = if (doPipeline) result.map { case (t, i) => (RegNext(t), i) }
        else result
        // recursive call
...
object BinaryTree {
  ...
  def apply[T <: Data](input: IndexedSeq[T], operator: (T, T) => T, pipelineInterval: Int = 0): BinaryTreeWithInfo[T, Null] = {
    ...
    val paddedOperator = (left: (T, Null), right: (T, Null)) => (operator(left._1, right._1), null)

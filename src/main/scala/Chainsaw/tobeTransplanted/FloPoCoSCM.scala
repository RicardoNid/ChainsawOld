...
import Chainsaw.tobeTransplanted.AOperations.AOpHardware
import Chainsaw.{tobeTransplanted, _}
...
class FloPoCoSCM(input: Real, constant: Int) extends ImplicitArea[Real] with Testable {
  ...
  opStrings.foreach { line =>
    ...
    if (!path.contains(coeffSum.abs)) {
      ...
      println(tobeTransplanted.AOpConfig(shift.toInt, 0, 0, sign))
      AOpConfigs += tobeTransplanted.AOpConfig(slLeft, slRight, 0, sign)
    }
  }
...
object FloPoCoSCM {
  ...
  def apply(input: Real, contant: Int): FloPoCoSCM = new FloPoCoSCM(input, contant)

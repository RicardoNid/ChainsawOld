...
case class AdderConfig(
    ...
    hasCIn: Boolean       = true,
    hasCOut: Boolean      = true,
    signed: Boolean       = true,
    hasOverflow: Boolean  = true,
    doSaturation: Boolean = false
) {
...
case class Adder(config: AdderConfig) extends Component {
  ...
  // determine cOut
  if (hasCOut)
    if (signed) io.cOut := (!coreAdder.cSMS & (io.x.msb | io.y.msb)) | (coreAdder.cSMS & io.x.msb & io.y.msb)

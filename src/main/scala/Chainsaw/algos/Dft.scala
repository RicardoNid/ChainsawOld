...
object Dft {
  ...
  /** generic DFT which can be implemented in multiple different domain
    *
    * @param omega
    *   N-th root of unity in the domain
    * @param unity
    *   multiplication unity in the domain
    * @param mult
    *   multiplication operation
    * @param add
    *   addition operation
    * @see
    *   ''Fast Algorithms for Signal Processing'' Chap1.4
    */
  @definition
  ...
  /** radix-2 fft
    ...
    */
  @fastAlgo
  def genericR2Fft[T](data: DenseVector[T], omega: T, inverse: Boolean = false, DIT: Boolean = false)(implicit
      semiring: Semiring[T],
      classTag: ClassTag[T]
  ): DenseVector[T] = ??? // TODO
}

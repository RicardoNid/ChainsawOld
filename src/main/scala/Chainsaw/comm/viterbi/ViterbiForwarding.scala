...
case class ViterbiForwarding(trellis: Trellis[Int]) extends Component {
  ...
  val sparse2dense: Map[(Int, Int), Int] = Array
    .tabulate(numStates, numStates)((r, c) => (r, c, matrix(r)(c)))
    .flatten
    .filter(_._3 < MinplusMatrix.max)
    .zipWithIndex
    .map { case ((r, c, value), i) => (r, c) -> i }
    .toMap

  val ret = Array
    .tabulate(1, numStates) { (i, k) =>
      // pick up signals

...
object Refs {
  ...
  def qamdemod(input: Array[BComplex], bitPerSymbol: Int, gray: Boolean = true): Array[Int] = {
    val ret =
      if (!gray) eng.feval[Array[Double]]("qamdemod", input.map(_.toMComplex), Array(1 << bitPerSymbol), "bin")
  ...
  def qamdemod(input: BComplex, bitPerSymbol: Int, gray: Boolean) = {
    val ret =
      if (!gray) eng.feval[Double]("qamdemod", input.toMComplex, Array(1 << bitPerSymbol), "bin")
  ...
  def getQAMValues(bits: Int, gray: Boolean = true) = {
    ...
    if (bits == 1) Array(BComplex(-1, 0), BComplex(1, 0))
    else { // caution: matlab default = gray
      val ret =

...
/** Wrapper of qamdemod, supporting bitAlloc and powAlloc
  *
  * @param bitAlloc
  *   the list of symOrder for each segment
  * @param powAlloc
  *   the list of power allocation from each symbol
  * @param energyType
  *   to specify the format of input energy value
  * @param symbolType
  *   to specify the format of output QAM symbols as fixed point complex number
  * @param gray
  *   symbol order, binary when false, gray when true
  * @param customSymbols
  *   when you need to specify custom QAM values(different from Matlab)
  */
case class QAMDeMod(
    bitAlloc: Seq[Int],
    powAlloc: Seq[Double],
    symbolType: HardType[ComplexNumber],
    gray: Boolean                          = true,

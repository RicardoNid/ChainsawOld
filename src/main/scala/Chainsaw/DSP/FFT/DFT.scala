...
case class DFT(N: Int, dataWidth: Int, coeffWidth: Int) extends Component with CustomPrecision {
  ...
  N match {
    ...
    case 2 => dataOut := Vec(Seq(dataIn(0) + dataIn(1), dataIn(0) - dataIn(1)).map(RegNext(_)))
    // TODO: implement radix-4 and radix-8 in a more complex way
    case 4 => {
    ...
    case 8 => {
      ...
      dataOut := RegNext(Vec(Seq(D0, D1, D2, D3, D4, D5, D6, D7)))
    }
